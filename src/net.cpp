...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Agroscoin " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
    0x00100007f, 0x422a3932, 0x2d206eb4,
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Agroscoin is probably already running."), addrBind.ToString().c_str());
        else
