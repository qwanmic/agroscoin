...
#if defined(NDEBUG)
# error "Agroscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4066eb0ca429d306a85074d9d27cad3f113bd266150acb94609551cec08dec8c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Agroscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Agroscoin Signed Message:\n";
...
{
    // Agroscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 84000000); // Agroscoin: 84000k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.35 * 24 * 60 * 60; // Agroscoin: 0.35
static const int64 nTargetSpacing = 15; // Agroscoin: 15 sec
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Agroscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Agroscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6352f45769f4ec2b6b209e9de3a1cb65f99de66962fd77f0b33151b0ef9037f9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Buat coin Agros untuk Indonesia agros bisnis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043a978f0e574243b28c436b6030587151dccbea7c5171bacaa48c0423a5d98550a2471f0562d897522e8c6bdf4ad449c7e373bf660c31e041892cb688bced8cea") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1534550109;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085464054;

        if (fTestNet)
        {
            block.nTime    = 1534550048;
            block.nNonce   = 387905887;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf88ed248f24433b1e95d3df8a713e83a15d07195f5ed7c1f2fb8c0c9498b5a37"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Agroscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AgroscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AgroscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AgroscoinMiner(CWallet *pwallet)
{
    printf("AgroscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("agroscoin-miner");
    ...
    try { loop {
        ...
        printf("Running AgroscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AgroscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AgroscoinMiner, pwallet));
}
